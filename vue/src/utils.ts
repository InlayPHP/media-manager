...
export const idEquals = (left: MediaId | null | undefined, right: MediaId | null | undefined) => left == null || right == null ? left == null && right == null : String(left) === String(right)
export const resourceAssets = (assets: MediaManagerResource['assets']) => Array.isArray(assets) ? assets : assets.data
export const resourcePagination = (assets: MediaManagerResource['assets']): MediaPagination | null => Array.isArray(assets) ? null : assets
...
export function endpointFor(endpoints: MediaEndpoints | undefined, key: keyof MediaEndpoints, id?: MediaId) {
  ...
  const encoded = id == null ? '' : encodeURIComponent(String(id))
  return id == null ? endpoint : endpoint.replace('{asset}', encoded).replace('{folder}', encoded).replace('{id}', encoded).replace('__ASSET__', encoded).replace('__FOLDER__', encoded)
}
